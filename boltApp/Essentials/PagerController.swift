...
open class PagerController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
	...
	open var tabsViewBackgroundColor: UIColor = UIColor(red:249.0/255.0, green: 53.0/255.0, blue: 55.0/255.0, alpha:1)
	open var tabsTextColor: UIColor = UIColor.white
	open var selectedTabTextColor = UIColor.white
    open var selectedTabBackgroundColor = UIColor(red:249.0/255.0, green: 53.0/255.0, blue: 55.0/255.0, alpha:1)
	...
	open var tabHeight: CGFloat = 60.0
	...
	func updateSelectedTab(_ index: Int) {
		...
		for tab in self.tabs {
			...
			if let label = tab?.subviews.first as? UILabel {
				label.textColor = tabsTextColor
                tab?.backgroundColor = tabsViewBackgroundColor
			}
		}
		...
		if let label = tab?.subviews.first as? UILabel {
            ...
            tab?.backgroundColor = selectedTabBackgroundColor
		}
	}
